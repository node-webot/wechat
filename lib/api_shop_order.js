 ...
 */
exports._getOrdersByStatus = function () {
  var data = {};
  var callback;
  if (arguments.length === 1 && typeof arguments[0] === 'function') {
    ...
    if (typeof arguments[0] === 'number') {
      ...
      data['begintime'] = Math.round(arguments[0].getTime() / 1000);
    ...
    if (typeof arguments[0] === 'number' && arguments[1] instanceof Date) {
      data['status'] = arguments[0];
      data['begintime'] = Math.round(arguments[1].getTime() / 1000);
      data['endtime'] = Math.round(new Date().getTime() / 1000);
    } else {
    ...
    if (typeof arguments[0] === 'number' && arguments[1] instanceof Date  && arguments[2] instanceof Date) {
      data['status'] = arguments[0];
      data['begintime'] = Math.round(arguments[1].getTime() / 1000);
      data['endtime'] = Math.round(arguments[2].getTime() / 1000);
    } else {
      callback(new Error('first parameter must be Number, second and third parameter must be Date'));
    }
  } else {
    throw new Error('Parameters should be 1-4, last parameter must be callback function!');
  }
 ...
 * 关闭订单的未封装版本
 */
exports._closeOrder = function (orderId) {
  var data = {
    "order_id": orderId
  };
