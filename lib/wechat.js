...
var respond = function (handler) {
  return function (req, res, next) {
    getMessage(req, function (err, result) {
      ...
      // 将消息交给第三方微信服务商处理
      res.transfer2OtherWechatService = function (url, token, defaultContent, timeout) {
        var q = {
          token: token,
        ...
        var options = {
          method: 'POST',
          content: req.weixin_xml,
          timeout: timeout
        };
        urllib.request(url, options, function (err, data, response) {
          if (err || response.statusCode !== 200 || data.toString('utf-8').indexOf('xml') === -1) {
            return res.reply(defaultContent || '');
          }
          res.writeHead(200);
          res.end(data);
        });
