  ...
  '<% if (MsgType === "news") { %>',
    ...
    '<% Articles.forEach(function(item){ %>',
      '<item>',
        ...
        '<PicUrl><![CDATA[<%-item.PicUrl || item.Picurl || item.picUrl %>]]></PicUrl>',
  ...
  return function (req, res, next) {
    getMessage(req, function (err, result) {
      ...
      res.reply = function (content, funcFlag) {
        ...
        info.MsgType = content.MsgType;

        if (info.MsgType === 'text') {
          info.Content = content.Content;
        } else if (info.MsgType === 'news') {
          info.ArticleCount = content.ArticleCount;
          info.Articles = content.Articles;
        } else if (info.MsgType === 'music') {
          info.Music = content.Music;
        }
