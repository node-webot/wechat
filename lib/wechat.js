...
var tpl = ['<xml>',
  ...
  '<% } else if (msgType === "transfer_customer_service") { %>',
    '<% if (content.kfAccount || content.KfAccount) { %>',
      '<TransInfo>',
...
var reply2CustomerService = function (kfAccount, fromUsername, toUsername) {
  var info = {};
  ...
  return function (req, res, next) {
    getMessage(req, function (err, result) {
      ...
      res.transfer2CustomerService = function (kfAccount) {
        res.writeHead(200);
        res.end(reply2CustomerService(kfAccount, message.ToUserName, message.FromUserName));
      };
