  ...
  return function (req, res, next) {
    ...
    var done = function () {
      ...
      if (message.MsgType === 'text' && req.wxsession && req.wxsession._wait) {
        var list = null;
        if (req.wxsession._list) {
          list = List.unserialize(req.wxsession._list);
        }
        if (!list) {
          list = List.get(req.wxsession._wait);
        }
        if (list) {
          var handle = list.get(message.Content);
          predefinedValue = list.getValue(message.Content);
          ...
          console.error('[wechat]session waitId ['+req.wxsession._wait+']have no list to reply');
        }
      ...
      // 兼容旧API
      if (handler.handle) {
        callback(req, res, next,predefinedValue);
      } else {
        callback(message, req, res,next,predefinedValue);
      }
    };
